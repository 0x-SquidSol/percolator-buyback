...
- New helper: `total_protocol_exposure(markets: &[MarketView]) -> Result<u128, BuybackBlocker>`. Applies the per-market formula in section 3.1 over the market's view(s) — one market per buyback check. Uses checked arithmetic and returns `Err(BuybackBlocker::MathOverflow)` on overflow rather than saturating — saturation is indistinguishable from genuine high exposure in operator logs, whereas an explicit `Err` preserves observability while keeping the same fail-closed property (the gate blocks regardless). Overflow at u128 scale still means something is wrong with input data, not that the buyback should fire.
