...
- **Slab vs market aggregation — per-slab.** The insurance fund lives inside each slab's `RiskEngine`; there is no global account today. With a single live market at launch, per-slab is mathematically equivalent to the global formulation in section 3. The implementation enforces a hardcoded `MAX_MARKETS_FOR_PER_SLAB = 1` invariant: a second live market causes `trigger_buyback` to fail closed with `BuybackBlocker::MultiMarketRequiresGlobalAggregation` until a program upgrade migrates to global aggregation (or the shared-vault concept tracked under PERC-628 ships, whichever comes first). This converts the "what happens at N=2" question into a forced upgrade gate rather than an emergent failure.
