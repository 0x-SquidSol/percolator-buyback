// VENDORED FROM dcccrypto/percolator-sdk/src/abi/encode.ts — DO NOT TRANSFER.
//
// This file exists in the staging crate ONLY so the buyback source files
// (src/solana/stake-buyback.ts, src/events/buyback.ts, etc.) can import
// the same byte-encoding primitives the destination already provides.
// The destination's own encode.ts is the authoritative copy; mirroring it
// here lets `tsc --noEmit` and `vitest run` work in this repo without
// fork-and-translate gymnastics. At transfer time, this file does NOT
// cross over — the integrator's imports resolve to the destination's
// existing encode.ts. If the destination's encode.ts is updated upstream,
// re-vendor this file (single `gh api` fetch) and delete the divergence.
