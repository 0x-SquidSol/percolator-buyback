...
The choice between **staged** and **spec-ready** is not stylistic — it follows from what can be reproduced in isolation. Pure libraries (math primitives) and off-chain services (TypeScript SDKs, keepers, indexers) can be built and tested here against stdlib or vendored dependencies, so this repo holds an authoritative reference implementation that transfers as a file copy plus a one-line module wiring. On-chain Rust programs (the wrapper, the vault) are intrinsically tied to the destination crate's instruction enum, processor scaffolding, error types, and account-validation conventions; reproducing those scaffolds here would mean forking the destination crate, so the entry stays at **spec-ready** until merged and the prose serves as the integrator's blueprint. The presence or absence of a `crates/<name>-staging/` path under the entry signals which side of the line that entry sits on.
